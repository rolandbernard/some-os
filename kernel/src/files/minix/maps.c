...
static Error genericMinixGetBitMap(MinixFilesystem* fs, size_t offset, size_t size, bool inode, size_t* pos) {
    ...
    while (bytes > 0) {
        ...
        CHECKED(vfsReadAt(fs->block_device, NULL, virtPtrForKernel(tmp_buffer), tmp_size, offset, &tmp_size), {
        ...
        for (size_t i = 0; size > 0 && i < tmp_size; i++) {
            ...
            for (int j = 0; size > 0 && j < 8; j++) {
                if (((byte >> j) & 1) == 0) {
                    // This bit is free
                    tmp_buffer[i] |= (1 << j);
                    CHECKED(vfsWriteAt(fs->block_device, NULL, virtPtrForKernel(&tmp_buffer[i]), 1, offset + i, &tmp_size), {
                        unlockTaskLock(&fs->maps_lock);
                        dealloc(tmp_buffer);
                    });
                    if (tmp_size == 0) {
...
static Error genericMinixClearBitMap(MinixFilesystem* fs, size_t offset, size_t position) {
    lockTaskLock(&fs->maps_lock);
    char tmp_buffer[1];
    size_t size;
    CHECKED(
        vfsReadAt(fs->block_device, NULL, virtPtrForKernel(tmp_buffer), 1, offset + position / 8, &size),
        unlockTaskLock(&fs->maps_lock)
    );
    if (size == 0) {
        unlockTaskLock(&fs->maps_lock);
        ...
        CHECKED(
            vfsWriteAt(fs->block_device, NULL, virtPtrForKernel(tmp_buffer), 1, offset + position / 8, &size),
            unlockTaskLock(&fs->maps_lock)
        );
        unlockTaskLock(&fs->maps_lock);
        if (size == 0) {
